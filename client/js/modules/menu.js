...
var initialise_react = function (data) {
    data = data || {};
    ...
    SideMenu = React.createClass({
        ...
        componentWillMount: function () {
            var that = this;
            menuHook.update = function (newData) {
...
var processData = function (data_obj, init) {
    var data = clone(data_obj);
    ...
    var display_children_only = (currentSpace.substr(-5) === 'nxdoc');
    ...
    if (init) {
        initialise_react(data);
    }
    else {
        menuHook.update(data);
    }
};

var refresh = function (init) {
    var space = $('#content-body').attr('data-space');
