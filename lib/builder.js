...
const build = function(source_path, metadata, destination_path, repo_details) {
  ...
  const content_filter =
    process.env.NODE_ENV === 'development' && process.env.FILTER ? process.env.FILTER.split(',') : void 0;
  ...
  const sitemap_filename =
    branch === 'static' ? `sitemap-${branch}.xml` : `sitemap-${repo_id}${version_path ? '-' + version_path : ''}.xml`;
  ...
  if (config && config.site && config.site.spaces && config.site.spaces.length) {
    config.site.spaces.forEach(function(space) {
      const version_space_path = version_path ? space.space_path + '/' + version_path : space.space_path;
  ...
  return (
    metalsmith(metalsmith_working_path)
      ...
      .use(metalsmith_if(algolia_key, time('algolia done')))
      .use(
        sitemap({
