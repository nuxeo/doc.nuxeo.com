...
const pre_build = function (repo_details) {
  ...
  info('Starting pre-build:', source_path);

  info('Builder Version:', version);
  ...
  return new Promise(function (resolve, reject) {
    ...
    metalsmith(metalsmith_working_path)
      ...
      // Check frontmatter for malformed YAML and log errors without crashing the build
      .frontmatter(false)
      .use((files, metalsmith, done) => {
        Object.keys(files).forEach((filePath) => {
          if (!/\.(md|html)$/.test(filePath)) return;
          try {
            matter(files[filePath].contents.toString(), { throwOnError: true });
          } catch (err) {
            error(
              'Invalid frontmatter in file: %s - will be processed with empty metadata: %s',
              filePath,
              err.message
            );
          }
