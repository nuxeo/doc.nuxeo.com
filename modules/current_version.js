...
module.exports = function (config, path_only = true) {
    ...
    if (config.repositories) {
        // Get the repo_url
        const repo_url = exec('git config --get remote.origin.url', {encoding: 'utf8', cwd: cwd});
        // Only use the last part of the url to allow for protocol differences
        const repo_url_part = typeof repo_url === 'string' && repo_url.split('/').pop().trim() || '';
        // console.log('repo_url_part', repo_url_part);

        // Get the branch name
        const branch_name = exec("git branch | grep ^\\* | grep -oE '[a-zA-Z0-9_-]+$'", {encoding: 'utf8', cwd: cwd}).trim();
        ...
        .map((repo_id) => {
            ...
            return Object.keys(repo.branches).filter(branch_id => branch_id === branch_name)
            .map((branch_id) => {
